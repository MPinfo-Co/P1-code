    ...
    >
      ...
      <List sx={{ flex: 1, py: 1.5 }} disablePadding>
        <NavLink to="/ai-partner" style={{ textDecoration: 'none' }}>
          {({ isActive }) => (
            <ListItemButton sx={{ px: 3, py: 1.2, ...(isActive ? activeSx : { '&:hover': { bgcolor: '#1e293b' } }) }}>
              <ListItemIcon sx={iconSx}><SmartToyOutlinedIcon fontSize="small" /></ListItemIcon>
        ...
        <NavLink to="/kb" style={{ textDecoration: 'none' }}>
          {({ isActive }) => (
            <ListItemButton sx={{ px: 3, py: 1.2, ...(isActive ? activeSx : { '&:hover': { bgcolor: '#1e293b' } }) }}>
              <ListItemIcon sx={iconSx}><MenuBookOutlinedIcon fontSize="small" /></ListItemIcon>
        ...
        {canSeeSetting && (
          <>
            <ListItemButton onClick={() => setSettingOpen(o => !o)} sx={{ px: 3, py: 1.2, '&:hover': { bgcolor: '#1e293b' } }}>
              <ListItemIcon sx={iconSx}><SettingsOutlinedIcon fontSize="small" /></ListItemIcon>
            ...
            <Collapse in={settingOpen} timeout="auto" unmountOnExit>
              <List disablePadding sx={{ bgcolor: '#0b1120' }}>
                {[
                  { to: '/settings/account', label: '帳號', icon: <ManageAccountsOutlinedIcon fontSize="small" /> },
                  { to: '/settings/role', label: '角色', icon: <GroupsOutlinedIcon fontSize="small" /> },
                  { to: '/settings/ai-config', label: 'AI夥伴管理', icon: <TuneOutlinedIcon fontSize="small" /> },
                ].map(item => (
                  <NavLink key={item.to} to={item.to} style={{ textDecoration: 'none' }}>
                    {({ isActive }) => (
                      <ListItemButton sx={{ pl: 7, py: 1, ...(isActive ? activeSx : { '&:hover': { bgcolor: '#1e293b' } }) }}>
                        <ListItemIcon sx={{ ...iconSx, minWidth: 28 }}>{item.icon}</ListItemIcon>
                        <ListItemText primary={item.label} sx={{ '& .MuiListItemText-primary': { fontSize: 13, color: isActive ? 'white' : '#94a3b8' } }} />
                      </ListItemButton>
                    )}
                  </NavLink>
                ))}
