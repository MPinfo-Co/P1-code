...
export default function FnToolForm({ open, row, onClose, onSuccess }: Props) {
  ...
  const showImageFieldsSection = toolType === 'image_extract'
  const showBodyParams = showConnectionSection && (httpMethod === 'POST' || httpMethod === 'PUT')
  ...
  async function handleSave() {
    ...
    if (toolType === 'external_api' && !endpointUrl.trim()) {
    ...
    try {
      if (isEdit && row) {
        const payload: import('@/queries/useToolsQuery').UpdateToolPayload = {
          name: name.trim(),
          ...
          ...(toolType === 'external_api'
            ...
            : toolType === 'image_extract'
              ? { image_fields: imageFields }
              : {}),
        }
        await updateTool.mutateAsync({ id: row.id, payload })
      } else {
        const payload: import('@/queries/useToolsQuery').CreateToolPayload = {
          name: name.trim(),
          description: description.trim(),
          tool_type: toolType,
          ...(toolType === 'external_api'
      ...
      <DialogContent sx={{ pt: '16px !important' }}>
        <Box className="fn-tool-form-stack">
          ...
          <Box>
            ...
            {isEdit ? (
              <Typography sx={{ fontSize: 14, color: '#334155', mt: 0.5 }}>
                {toolType === 'external_api'
                  ? 'API 呼叫'
                  : toolType === 'image_extract'
                    ? '圖片擷取'
          ...
          {/* 擷取欄位定義區段（工具類型為圖片擷取時顯示） */}
          {showImageFieldsSection && (
            <>
