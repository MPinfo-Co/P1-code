  ...
  if (!issue) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-slate-400">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-[#2e3f6e] text-sm font-semibold hover:underline">← 返回</button>
      </div>
    )
  }
      ...
      <div style={{ display: 'flex', gap: 20, flex: 1, overflow: 'hidden', minHeight: 0 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          ...
                          <div style={{ flexShrink: 0, paddingTop: 1 }}>
                            <button onClick={() => askSuggestStep(idx)}
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 8px', borderRadius: 4, border: '1px solid #2e3f6e', background: 'white', color: '#2e3f6e', fontSize: 11, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s' }}
                  ...
                  <div style={{ background: '#f8fafc' }}>
                    ...
                    {issue.logs.length > 3 && (
                      <button onClick={() => setShowAllLogs(v => !v)}
                        style={{ marginTop: 10, fontSize: 12, padding: '4px 10px', border: '1px solid #cbd5e1', background: 'white', borderRadius: 6, cursor: 'pointer', fontWeight: 600 }}>
            ...
            {activeTab === 'diary' && (
              <div>
                ...
                <div style={{ marginBottom: 14 }}>
                  {!issue.history || issue.history.length === 0 ? (
                    ...
                    <div key={idx} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '6px 0', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <span onClick={() => deleteHistoryEntry(idx)} title="刪除"
                        style={{ cursor: 'pointer', flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 22, height: 22, borderRadius: 4, color: '#94a3b8', transition: 'all 0.15s' }}
                ...
                {showResolution && (
                  <div style={{ display: 'block', marginTop: 12, padding: '12px 14px', background: '#f0fdf4', border: '1.5px solid #86efac', borderRadius: 8, marginBottom: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
                      <div style={{ flex: 1 }}>
                        <label style={{ fontSize: 11, color: '#166534', fontWeight: 700, display: 'block', marginBottom: 3 }}>處理人</label>
                        <input type="text" value={resHandler} onChange={e => setResHandler(e.target.value)} placeholder="處理人姓名"
                          style={{ width: '100%', boxSizing: 'border-box', height: 32, padding: '0 8px', border: '1.5px solid #86efac', borderRadius: 6, fontSize: 13, outline: 'none', background: 'white', color: '#1e293b' }} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <label style={{ fontSize: 11, color: '#166534', fontWeight: 700, display: 'block', marginBottom: 3 }}>處置時間</label>
                    ...
                    <textarea value={resText} onChange={e => setResText(e.target.value)} rows={3}
                      ...
                      style={{ width: '100%', boxSizing: 'border-box', padding: '8px 10px', border: '1.5px solid #86efac', borderRadius: 6, fontFamily: 'inherit', fontSize: 13, outline: 'none', resize: 'vertical', background: 'white', color: '#1e293b' }} />
                    <button onClick={saveResolution}
                      style={{ marginTop: 8, background: '#166534', color: 'white', border: 'none', borderRadius: 6, fontSize: 12, padding: '6px 14px', cursor: 'pointer', fontWeight: 700 }}>
                      儲存結案說明
                    </button>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'nowrap', alignItems: 'flex-end', padding: 12, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                  <div>
                    <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 3 }}>日期</label>
                    <input type="datetime-local" value={histDate} onChange={e => setHistDate(e.target.value)}
                      style={{ height: 36, padding: '0 8px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 3 }}>備註</label>
                    <input type="text" value={histNote} onChange={e => setHistNote(e.target.value)} placeholder="輸入處理動作備註..."
                      style={{ height: 36, width: '100%', padding: '0 8px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 3 }}>變更狀態</label>
                    <select value={histStatus} onChange={e => setHistStatus(e.target.value)}
                      style={{ height: 36, padding: '0 8px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, outline: 'none', boxSizing: 'border-box', minWidth: 100 }}>
                      {['未處理', '處理中', '已完成', '擱置'].map(s => <option key={s}>{s}</option>)}
                    </select>
                  </div>
                  <button onClick={addHistoryEntry}
                    style={{ height: 36, padding: '0 14px', background: '#2e3f6e', color: 'white', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                    新增
                  </button>
                </div>
              </div>
        ...
        <div style={{ width: chatVisible ? 360 : 40, flexShrink: 0, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden', transition: 'width 0.2s' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => setChatVisible(v => !v)} title={chatVisible ? '收合' : '展開'}
              style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: 6, border: '1.5px solid #cbd5e1', background: 'white', flexShrink: 0, marginLeft: chatVisible ? 0 : 'auto' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#eef1f8'; e.currentTarget.style.borderColor = '#2e3f6e' }}
            ...
            <div style={{ background: 'white', borderRadius: 12, display: 'flex', flexDirection: 'column', border: '1px solid #e2e8f0', minHeight: 0, overflow: 'hidden', flex: 1 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5, padding: '8px 10px', borderTop: '1px solid #e2e8f0', background: '#f8fafc' }}>
                {['整體摘要', '相關發現', '補充說明', '修復建議'].map(type => (
                  <button key={type} onClick={() => sendQuickChat(type)}
                    style={{ padding: '7px 10px', borderRadius: 7, border: '1.5px solid #c5ccdf', background: 'white', color: '#2e3f6e', fontSize: 12, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s', whiteSpace: 'nowrap', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 5 }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#eef1f8'; e.currentTarget.style.borderColor = '#2e3f6e' }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'white'; e.currentTarget.style.borderColor = '#c5ccdf' }}>
                    {type}
                  </button>
                ))}
              </div>
              ...
              <div style={{ padding: 12, borderTop: '1px solid #e2e8f0', display: 'flex', gap: 8, alignItems: 'center', background: '#f8fafc', borderRadius: '0 0 12px 12px' }}>
                <input
                  ...
                  style={{ flex: 1, padding: '10px 16px', border: '1px solid #cbd5e1', borderRadius: 20, outline: 'none', fontFamily: 'inherit', fontSize: 13 }}
                />
                <button onClick={() => sendMsg()}
                  style={{ background: '#2e3f6e', color: 'white', border: 'none', width: 38, height: 38, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
