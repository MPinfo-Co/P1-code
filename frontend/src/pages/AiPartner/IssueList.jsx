...
function StarRank({ rank }) {
  ...
  return (
    <span style={{ whiteSpace: 'nowrap' }}>
      ...
      <span style={{ color: '#e2e8f0', fontSize: 15, letterSpacing: 1 }}>{'★'.repeat(5 - rank)}</span>
    </span>
  )
}

function StatusIcon({ status }) {
  if (status === '未處理') return (
    ...
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <div
        ...
        style={{
          ...
          background: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', cursor: 'pointer', whiteSpace: 'nowrap'
        }}
      ...
      {open && (
        <div style={{
          display: 'block', position: 'absolute', top: 'calc(100% + 6px)', left: 0, zIndex: 200,
          background: 'white', border: '1px solid #cbd5e1', borderRadius: 8, padding: '12px 14px',
          minWidth: 280, maxWidth: 420, boxShadow: '0 8px 24px rgba(0,0,0,0.12)',
          fontSize: 13, color: '#334155', lineHeight: 1.6, whiteSpace: 'normal'
        }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>完整影響範圍</div>
          {issue.affected}
        </div>
      )}
    </div>
...
export default function IssueList() {
  ...
  function resetFilters() {
    setFilterStatus('all'); setFilterStart(''); setFilterEnd(''); setFilterKeyword('')
  ...
  function getAssignee(issue) {
    if (issue.history && issue.history.length > 0) return issue.history[issue.history.length - 1].user
    return null
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 110px)' }}>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4 flex-shrink-0 flex flex-wrap gap-3 items-center">
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          style={{ height: 36, padding: '0 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, outline: 'none', minWidth: 90 }}>
          <option value="all">全部</option>
          {['未處理', '處理中', '已完成', '擱置'].map(s => <option key={s}>{s}</option>)}
        </select>
        <label className="text-sm whitespace-nowrap">發生日期:</label>
        <input type="date" value={filterStart} onChange={e => setFilterStart(e.target.value)}
          style={{ height: 36, padding: '0 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
        ...
        <input type="text" value={filterKeyword} onChange={e => setFilterKeyword(e.target.value)}
          ...
          style={{ height: 36, padding: '0 12px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, outline: 'none', boxSizing: 'border-box', width: 200 }} />
        <button onClick={applyFilters}
          style={{ padding: '8px 16px', borderRadius: 6, cursor: 'pointer', fontWeight: 600, border: '1.5px solid #2e3f6e', background: 'white', color: '#2e3f6e', fontSize: 13.5 }}
          onMouseEnter={e => e.currentTarget.style.background = '#eef1f8'}
          onMouseLeave={e => e.currentTarget.style.background = 'white'}>
          套用
        </button>
        <button onClick={resetFilters} className="px-4 py-2 border border-slate-300 bg-white text-slate-700 rounded-md text-sm font-semibold hover:bg-slate-50">重設</button>
      </div>
        ...
        <div style={{ height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex-shrink-0">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {['事件說明', '處理優先級', '發生日期區間', '影響範圍', '處理狀態', '負責人員', '執行動作'].map(h => (
                    <th key={h} className="text-left px-4 py-3.5 bg-slate-100 text-slate-800 font-extrabold text-sm border-b-2 border-slate-300">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {paged.map(issue => (
                  <tr key={issue.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors cursor-pointer"
                    ...
                    <td className="px-4 py-3.5 text-sm text-slate-600 whitespace-nowrap">
                      {issue.dateEnd && issue.dateEnd !== issue.date ? `${issue.date} ~ ${issue.dateEnd}` : issue.date}
                    </td>
                    <td className="px-4 py-3.5">
                      <AffectedPopover issue={issue} />
                    </td>
                    <td className="px-4 py-3.5 text-sm">
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 500, color: '#334155' }}>
                    ...
                    <td className="px-4 py-3.5">
                      <button
                        onClick={e => { e.stopPropagation(); navigate(`/ai-partner/${partnerId}/issues/${issue.id}`) }}
