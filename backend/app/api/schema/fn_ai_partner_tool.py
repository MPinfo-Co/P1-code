...
class ToolCreate(BaseModel):
    ...
    endpoint_url: str | None = Field(
        None, description="API Endpoint URL（external_api 類型必填）"
    )
    http_method: str | None = Field(
        None, description="HTTP Method（external_api 類型必填）"
    )
    auth_type: str | None = Field(None, description="認證方式：none / api_key / bearer")
    ...
    target_url: str | None = Field(None, description="目標網址（web_scraper 類型必填）")
    extract_description: str | None = Field(
        None, description="擷取描述（web_scraper 類型必填）"
    )
    max_chars: int = Field(
        default=4000, description="最大擷取字元數（web_scraper 類型）"
    )
