...
def _sync_settings() -> None:
    ...
    try:
        ...
        if row is None:
            ...
            return
        logger.info(f"Schedule settings: enable: {row.is_enabled}, schedule_time: {row.schedule_time}, ssb_host: {row.ssb_host}, ssb_port: {row.ssb_port}, ssb_logspace: {row.ssb_logspace}")
